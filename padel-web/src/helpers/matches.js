 ...
 *  (fijos, forced, draft global, rellenar incompletos, ordenar por pistas…).
 *  Para no duplicar, se importan utilidades comunes y pesos.
 */
...
import {
  ...
  export function fillFixedGroup(group, pool, opciones) {
    ...
    /* todas las combinaciones posibles que no violen “no juega con / contra” */
    ...
    /* mantener 2H-2M en mixto perfecto */
    if (esMixtoPerfecto) {
    ...
    /* escorar por suma de nivel – rango interno (más regular = mejor) */
    ...
    /* quita del pool */
    const idsToRemove = new Set(pick.combo.map((p) => p.id));
