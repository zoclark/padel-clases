...
export default function PozoParticipantes({ pozoId, onParticipantesActualizados }) {
  ...
  const handleExcelUpload = async () => {
    ...
    try {
      ...
      toast.success(data?.mensaje || "Participantes importados");
      setExcelFile(null);
      refreshLista();
    } catch (err) {
      const msg =
              ...
              <>
                ...
                <div onClick={(e) => e.stopPropagation()} className="space-y-1">
                  ...
                  <select
                    value={edicion.nivel}
                    onChange={(e) =>
                      setEdicion((prev) => ({ ...prev, nivel: e.target.value }))
                    }
                    className="w-full p-1 rounded text-black text-xs"
                  >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEdicion((prev) => ({ ...prev, genero: e.target.value }))
                    }
                    className="w-full p-1 rounded text-black text-xs"
                  >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEdicion((prev) => ({ ...prev, posicion: e.target.value }))
                    }
                    className="w-full p-1 rounded text-black text-xs"
                  >
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setEdicion((prev) => ({
                        ...prev,
                        pista_fija: val === "" ? null : Number(val),
                      }));
                    }}
                    className="w-full p-1 rounded text-black text-xs"
                  >
              ...
              <>
                ...
                <div>
                  ...
                  <div className="mt-1 text-[10px] space-y-0.5">
                    ...
                    <div>
                      Género: {p.genero === "hombre" ? "👦 Hombre" : "👧 Mujer"}
                    </div>
                    <div>
        ...
        <div className="max-w-lg mx-auto mb-6 text-white">
          ...
          <ul className="space-y-4">
            {emparejamientos.map((m, i) => (
              <li key={i}>
              <strong>Pista {m.pista}</strong>
              <br />
              Pareja A: {m.teams[0][0]} + {m.teams[0][1]} → total {m.totals[0]}, media {m.avgs[0]}
              <br />
              Pareja B: {m.teams[1][0]} + {m.teams[1][1]} → total {m.totals[1]}, media {m.avgs[1]}
              <br />
